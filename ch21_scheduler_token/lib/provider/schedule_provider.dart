...
class ScheduleProvider extends ChangeNotifier {
  ...
  Future<void> login({required String email, required String password}) async {
    ...
    updateTokens(accessToken: resp.accessToken, refreshToken: resp.refreshToken);
  }

  logout(){
    updateTokens(accessToken: null, refreshToken: null);
    cache = {};
    notifyListeners();
  }
  ...
  // 2가지의 토큰을 갱신하는 함수
  rotateToken({required String refreshToken, required bool isRefreshToken}) async {
    if(isRefreshToken) {
  ...
  void createSchedule({required ScheduleModel schedule}) async {
    ...
    cache.update(
      targetDate,
          (value) =>
      [...value, newSchedule] // [기존 일정들, 새로운 일정을 추가]
      //추가 후에 시작 시간 순으로 정렬
  ...
  void deleteSchedule({required DateTime date, required String id}) async {
    ...
    try {
      await scheduleRepository.deleteSchedule(id: id, accessToken: accessToken!); // ➊ 삭제함수 실행
    } catch (e) {
      cache.update(
        // ➋ 삭제 실패시 캐시 롤백하기
        date,
